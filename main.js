...
//scraper by me
//importing for now to test, will be a fetch call in injected code
...
async function scrape() {
    ...
    let courses = scheduleJSON.sections;
    let term = scheduleJSON.term;

    //loop over courses and get info
    let days = new Map([
        ["sunday", 'SU'],
        ["monday", 'MO'],
        ["tuesday", 'TU'],
    ...
    for (let i = 0; i < courses.length; i++) {
        ...
        only adds courses with meeting times.
        */
        if (!course.meetings[0].no_meeting) {
            ...
            cal.addEvent(course_name, '', course.meetings[0].building, start_day + ' ' + start_time, start_day + ' ' + end_time, rrule);
        }
    }
    cal.download();
...
// (Otherwise we don't have access to https://my.uw.edu/api/v1/schedule/current)
// appendScript(import1);
// appendScript(import2);
// appendScript('('+scrape.toString()+')();');
// console.log('done');
